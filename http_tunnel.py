...
class TunnelThread(threading.Thread):
    ...
    def start_tunnel(self):
        try:
            ssh_tunnel = pexpect.spawn(tunnel_command, timeout=1200)
            ...
            command = "echo $'\nHTTP tunnel restarted, new processs info:\n%s' >> /path_to_log_file/http_tunnel_log" % output
            os.system(command)

            p = Popen('curl http://your_domain.com:your_port', shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True)
            output = p.stdout.read()

            ssh_tunnel.expect(pexpect.EOF)
	 
        except Exception, e:
            command = "echo $'\nHTTP TIMEOUT reached at : ' %s >> /path_to_log_file/http_tunnel_log" % time.asctime()
            os.system(command)
...
tunnel_command = '/usr/bin/ssh -NnTxi /root/cie.if -R your_port:localhost:80 -l your_user your_domain.com &'
host = 'your_domain'
